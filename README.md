...
Este mini jogo de ping pong foi desenvolvido utilizando o Pycharm, com a biblioteca do pygame.


<!--pong-->
<p>
  <img src="img/pong.gif" width="598" height="336">
</p>

A seguir segue o executável do jogo:

[Download](/jogo/game.exe)
...
## Renderização do Menu Principal do Jogo
```python
def main_menu():
    while True:
...
O trecho de código a seguir cria um loop contínuo para capturar eventos do usuário, como fechar a janela do jogo, pressionar teclas específicas e realizar ações correspondentes a esses eventos. Ele também para a reprodução de música do jogo e preenche a tela com uma cor preta.
```python
while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
...
def game_loop():
    ...
    while True:
        ...
        pygame.draw.aaline(screen, WHITE, (SCREEN_WIDTH // 2, 0), (SCREEN_WIDTH // 2, SCREEN_HEIGHT))  # Desenha uma linha vertical no centro da tela
...
## Final de Jogo
A função end_game lida com o término do jogo e a exibição da tela de fim de jogo. Ela verifica eventos de saída do jogo e teclas pressionadas. Se a tecla for a barra de espaço, o jogo é reiniciado. Se for a tecla Esc, o jogo é encerrado. A música é interrompida, a tela é preenchida de preto, e é exibido um texto de vitória na tela. A tela é atualizada e o jogo continua até um evento de saída ocorrer.
```python
def end_game(winner):
    while True:
...
## Reiniciando Jogo
O código a seguir realiza a reinicializção do jogo, posicionando os componentes em suas posições iniciais de jogo.
